...
def plot_zero_inflation_analysis(df, output_dir):
    ...
    bars = ax1.bar(categories, proportions, color=colors, edgecolor='black', linewidth=0.5)
    ...
    ax2.hist(positive_questions, bins=range(1, int(positive_questions.max())+2), 
    ...
    ax3.hist(positive_meetings, bins=bins, alpha=0.7, color='firebrick', 
    ...
    ax4.hist(np.log1p(positive_questions), bins=20, alpha=0.6, 
             label='Log(Perguntas+1)', color='steelblue', edgecolor='black', linewidth=0.5)
    ax4.hist(np.log1p(positive_meetings), bins=20, alpha=0.6, 
             label='Log(Reuniões+1)', color='firebrick', edgecolor='black', linewidth=0.5)
...
def plot_time_series_analysis(df, output_dir):
    ...
    ax1.plot(df_time['date'], df_time['questions_sum'], 'b-', linewidth=2.5, 
             label='Perguntas Totais', marker='o', markersize=3)
    ax1_twin = ax1.twinx()
    ax1_twin.plot(df_time['date'], df_time['meetings_sum'], 'r-', linewidth=2.5, 
                  label='Reuniões Totais', marker='s', markersize=3)
    
    ax1.set_title('Evolução Temporal: Totais Mensais')
    ax1.set_ylabel('Total de Perguntas', color='blue')
    ax1_twin.set_ylabel('Total de Reuniões', color='red')
    ax1.tick_params(axis='x', rotation=45)
    ax1.grid(True, alpha=0.3)
    
    # 2.2 Médias por MEP-domínio
    ax2.plot(df_time['date'], df_time['questions_mean'], 'b-', linewidth=2.5, 
             label='Média Perguntas', marker='o', markersize=3)
    ax2_twin = ax2.twinx()
    ax2_twin.plot(df_time['date'], df_time['meetings_mean'], 'r-', linewidth=2.5, 
                  label='Média Reuniões', marker='s', markersize=3)
    
    ax2.set_title('Evolução Temporal: Médias por MEP-Domínio')
    ax2.set_ylabel('Média de Perguntas', color='blue')
    ax2_twin.set_ylabel('Média de Reuniões', color='red')
    ax2.tick_params(axis='x', rotation=45)
    ax2.grid(True, alpha=0.3)
    
    # 2.3 Taxa de tratamento ao longo do tempo
    ax3.plot(df_time['date'], df_time['treated_mean'] * 100, 'g-', linewidth=2.5, 
             marker='d', markersize=4)
    ax3.set_title('Taxa de Tratamento ao Longo do Tempo')
    ax3.set_ylabel('% de MEP-Domínios com Reuniões')
    ...
    ax4.plot(df_time['date'], correlations, 'purple', linewidth=2.5, 
...
def plot_domain_heterogeneity(df, output_dir):
    ...
    bars1 = ax1.bar([x - 0.2 for x in x_pos], domain_stats['questions_mean'], 
                    0.4, label='Perguntas', color='steelblue', alpha=0.8)
    bars2 = ax1.bar([x + 0.2 for x in x_pos], domain_stats['meetings_mean'], 
                    0.4, label='Reuniões', color='firebrick', alpha=0.8)
    
    ax1.set_title('Atividade Média por Domínio de Política')
    ax1.set_ylabel('Média Mensal')
    ax1.set_xlabel('Domínio')
    ax1.set_xticks(x_pos)
    ax1.set_xticklabels([str(d)[:12] + '...' if len(str(d)) > 12 else str(d) 
                        for d in domain_stats['domain']], rotation=45, ha='right')
    ax1.legend()
    ax1.grid(True, alpha=0.3, axis='y')
    
    # 3.2 Taxa de tratamento por domínio
    bars = ax2.bar(x_pos, domain_stats['treated_mean'] * 100, 
    ...
    ax2.set_xticklabels([str(d)[:12] + '...' if len(str(d)) > 12 else str(d) 
                        for d in domain_stats['domain']], rotation=45, ha='right')
    ...
    domain_questions = [df[df['domain'] == domain]['questions'].values for domain in domains]
    
    bp1 = ax3.boxplot(domain_questions, labels=[str(d)[:8] + '...' if len(str(d)) > 8 else str(d) 
                                               for d in domains], patch_artist=True)
    ...
    domain_meetings = [df[(df['domain'] == domain) & (df['meetings'] > 0)]['meetings'].values 
                      for domain in domains]
    
    # Filtrar domínios que têm dados
    valid_meetings = [(meetings, domain) for meetings, domain in zip(domain_meetings, domains) 
                     if len(meetings) > 0]
    
    if valid_meetings:
        meetings_data, meeting_domains = zip(*valid_meetings)
        bp2 = ax4.boxplot(meetings_data, 
                         labels=[str(d)[:8] + '...' if len(str(d)) > 8 else str(d) 
                                for d in meeting_domains], patch_artist=True)
...
def plot_correlation_analysis(df, output_dir):
    ...
    ax1.scatter(sample_df['meetings'], sample_df['questions'], 
               alpha=0.3, s=8, color='darkblue')
    ...
    ax2.scatter(sample_log_meetings, sample_log_questions, 
               alpha=0.3, s=8, color='darkgreen')
    ...
    correlations_by_domain = df.groupby('domain').apply(
        lambda x: x['questions'].corr(x['meetings']) if len(x) > 1 else 0
    ).reset_index()
    correlations_by_domain.columns = ['domain', 'correlation']
    correlations_by_domain = correlations_by_domain.fillna(0)
    
    bars = ax3.bar(range(len(correlations_by_domain)), correlations_by_domain['correlation'],
    ...
    ax3.set_xticklabels([str(d)[:10] + '...' if len(str(d)) > 10 else str(d) 
                        for d in correlations_by_domain['domain']], rotation=45, ha='right')
...
def plot_extensive_intensive_margins(df, output_dir):
    ...
    crosstab = pd.crosstab(df['has_questions'], df['has_meetings'], normalize='all') * 100
    ...
    for i in range(len(crosstab.index)):
        for j in range(len(crosstab.columns)):
            text = ax1.text(j, i, f'{crosstab.iloc[i, j]:.1f}%',
                           ha="center", va="center", color="white" if crosstab.iloc[i, j] > 50 else "black",
    ...
    bars1 = ax2.bar(x - width/2, extensive_values, width, label='Extensiva (% com atividade)', 
                    color='steelblue', alpha=0.8)
    
    # Segundo eixo y para margem intensiva
    ax2_twin = ax2.twinx()
    bars2 = ax2_twin.bar(x + width/2, intensive_values, width, label='Intensiva (média se > 0)', 
    ...
    positive_questions = df[df['questions'] > 0]['questions']
    ax3.hist(positive_questions, bins=range(1, min(17, int(positive_questions.max())+2)), 
             alpha=0.7, color='steelblue', edgecolor='black', linewidth=0.5)
    ...
    ax3.axvline(mean_q, color='red', linestyle='--', linewidth=2, label=f'Média: {mean_q:.2f}')
    ax3.axvline(median_q, color='orange', linestyle='--', linewidth=2, label=f'Mediana: {median_q:.2f}')
    ax3.legend()
    ...
    ax4.hist(positive_meetings[positive_meetings <= max_meetings], bins=bins, 
             alpha=0.7, color='firebrick', edgecolor='black', linewidth=0.5)
    ...
    ax4.axvline(mean_m, color='blue', linestyle='--', linewidth=2, label=f'Média: {mean_m:.2f}')
    ax4.axvline(median_m, color='orange', linestyle='--', linewidth=2, label=f'Mediana: {median_m:.2f}')
    ax4.legend()
