...
class RobustDiDEstimator:
    ...
    def __init__(self, df, treatment_col, outcome_col, entity_col, time_col):
        """
        Initialize robust DiD estimator.
    ...
    def goodman_bacon_decomposition(self):
        ...
        for unit in self.df[self.entity_col].unique():
            ...
            if unit_data[self.treatment_col].max() > 0:
                # Find the first time when treatment is above threshold
                treatment_mask = unit_data[self.treatment_col] > unit_data[self.treatment_col].quantile(0.75)
                if treatment_mask.any():
                    first_treatment_idx = treatment_mask.idxmax()
                    treatment_timing[unit] = self.df.loc[first_treatment_idx, self.time_col]
                else:
        ...
        if treatment_timing:
            ...
            try:
                ...
                treatment_timing = {k: v for k, v in treatment_timing.items() if pd.notna(v)}
        ...
        for i, (unit1, time1) in enumerate(treatment_timing.items()):
            for j, (unit2, time2) in enumerate(treatment_timing.items()):
                if i < j:  # Avoid duplicates
                    ...
                    weight = self._calculate_comparison_weight(unit1, unit2, time1, time2)
                    effect = self._calculate_comparison_effect(unit1, unit2, time1, time2)
    ...
    def callaway_santanna_estimator(self, control_group="never_treated"):
        ...
        for unit, treat_time in treatment_timing.items():
            ...
            if control_group == "never_treated":
                ...
                control_units = [u for u, t in treatment_timing.items() if t > treat_time]
...
class EnhancedParallelTrends:
    ...
    def placebo_tests(self, n_placebos=100):
        ...
        for i in range(n_placebos):
            ...
            placebo_df['placebo_treatment'] = np.random.choice([0, 1], size=len(placebo_df))
    ...
    def plot_group_trends(self, pre_periods=6, treatment_threshold=1):
        ...
        for unit in df_plot[self.entity_col].unique():
            ...
            if len(high_lobbying_periods) > 0:
                ...
                unit_treatment_dates[unit] = df_plot.loc[first_treatment_idx, self.time_col]
            else:
        ...
        for unit, treat_time in unit_treatment_dates.items():
            if treat_time is not None:
                ...
                try:
                    if pd.api.types.is_datetime64_any_dtype(df_plot.loc[unit_mask, self.time_col]):
                        # If time column is datetime, convert to months since treatment
                        time_diff = (df_plot.loc[unit_mask, self.time_col] - treat_time).dt.days / 30
                    ...
                    print(f"Warning: Could not calculate relative time for unit {unit}: {e}")
                    # Fallback: use simple numeric conversion
                    try:
                        df_plot.loc[unit_mask, 'relative_time'] = (
                            pd.to_numeric(df_plot.loc[unit_mask, self.time_col], errors='coerce') - 
        ...
        # Summary statistics
        treated_units = len([u for u, t in unit_treatment_dates.items() if t is not None])
        total_units = len(unit_treatment_dates)
        
        print(f"Treated units: {treated_units} out of {total_units} ({treated_units/total_units*100:.1f}%)")
        
        # Calculate group means by relative time
        group_means = df_plot.groupby(['relative_time', 'ever_treated'])[self.outcome_col].mean().reset_index()
        ...
        if len(treated_data) > 0:
            ax.plot(treated_data['relative_time'], treated_data[self.outcome_col], 
                   marker='o', linewidth=2, markersize=6, label='Treated Group', color='blue')
        
        # Plot control group
        control_data = group_means[group_means['ever_treated'] == 0]
        if len(control_data) > 0:
            ax.plot(control_data['relative_time'], control_data[self.outcome_col], 
                   marker='s', linewidth=2, markersize=6, label='Control Group', color='red')
        
        # Add treatment line
        ax.axvline(x=0, color='black', linestyle='--', alpha=0.7, linewidth=2, label='Treatment')
        ...
        ax.set_title('Parallel Trends Assessment: Treated vs Control Groups', fontsize=14, fontweight='bold')
        ...
        print(f"Treatment threshold: {threshold:.4f}")
        print(f"Relative time range: {group_means['relative_time'].min():.1f} to {group_means['relative_time'].max():.1f}")
...
class ContinuousTreatmentEffects:
    ...
    def dose_response_function(self, n_quantiles=10):
        ...
        dose_response = self.df.groupby('treatment_quantile').agg({
        ...
        if len(X) < 3:
            print("Warning: Not enough data points for quadratic fit. Using linear model.")
    ...
    def generalized_propensity_score(self, covariates):
        ...
        gps_model = sm.OLS(self.df[self.treatment_col], 
                          sm.add_constant(self.df[covariates])).fit()
    ...
    def _plot_dose_response(self, dose_response, model):
        ...
        # Plot data points
        ax.scatter(dose_response[self.treatment_col], dose_response[self.outcome_col], 
                  alpha=0.7, s=50)
        
        # Plot fitted curve
        X_plot = np.linspace(dose_response[self.treatment_col].min(), 
                           dose_response[self.treatment_col].max(), 100)
...
class TopicClassificationValidator:
    ...
    def classification_accuracy(self, sample_size=100):
        ...
        for topic_col in self.topic_cols:
            ...
            precision = np.sum((predicted == 1) & (actual == 1)) / np.sum(predicted == 1)
        ...
        # Overall accuracy
        overall_accuracy = np.mean([results['accuracy'] for results in accuracy_results.values()])
        
        print(f"Overall classification accuracy: {overall_accuracy:.3f}")
        print(f"Topic-specific accuracies: {[f'{k}: {v['accuracy']:.3f}' for k, v in accuracy_results.items()]}")
    ...
    def _simulate_manual_coding(self, sample_size):
        ...
        for topic_col in self.topic_cols:
            ...
            base_classification = self.df[topic_col].sample(n=sample_size, random_state=42)
            noise = np.random.choice([0, 1], size=sample_size, p=[0.9, 0.1])  # 10% error rate
...
class CrossTopicSpilloverModel:
    ...
    def network_aware_did(self, network_structure=None):
        ...
        self.df['network_centrality'] = self._calculate_network_centrality(network_structure)
        
        # Network-aware treatment effects
        self.df['network_treatment'] = self.df[self.topic_cols].sum(axis=1) * self.df['network_centrality']
...
class InstrumentalVariablesEnhanced:
    ...
    def natural_experiment_iv(self, instrument_col, first_stage_controls=None):
        ...
        first_stage = sm.OLS(self.df[self.treatment_col], 
                            sm.add_constant(self.df[first_stage_vars])).fit()
        
        # Get predicted values
        self.df['treatment_hat'] = first_stage.predict()
        
        # Second stage: Outcome = f(Predicted_Treatment, Controls)
        second_stage = sm.OLS(self.df[self.outcome_col], 
                             sm.add_constant(self.df[['treatment_hat'] + first_stage_controls])).fit()
...
class EnhancedLobbyingEffectsModel:
    ...
    def set_topic(self, topic):
        ...
        if treatment_col not in available_cols:
            ...
            treatment_candidates = [col for col in available_cols if f"meetings_l_{topic}" in col]
        ...
        if outcome_col not in available_cols:
            # Look for similar columns
            outcome_candidates = [col for col in available_cols if f"questions_infered_topic_{topic}" in col]
            if outcome_candidates:
                outcome_col = outcome_candidates[0]
            else:
                # Use first available questions column
                questions_cols = [col for col in available_cols if "questions_infered_topic_" in col]
        ...
        if entity_col not in self.df.index.names:
            entity_col = self.df.index.names[0] if len(self.df.index.names) > 0 else "member_id"
        ...
        if treatment_col not in working_df.columns:
            print(f"Warning: Treatment column '{treatment_col}' not found. Using first available column.")
            treatment_col = working_df.columns[0]
        
        if outcome_col not in working_df.columns:
            print(f"Warning: Outcome column '{outcome_col}' not found. Using second available column.")
            outcome_col = working_df.columns[1] if len(working_df.columns) > 1 else working_df.columns[0]
        ...
        if time_col not in working_df.columns:
            time_col = working_df.columns[1] if len(working_df.columns) > 1 else working_df.columns[0]
        ...
        topic_cols = [col for col in working_df.columns if "questions_infered_topic_" in col]
        if not topic_cols:
    ...
    def _run_robust_did_analysis(self):
        ...
        if self.robust_did is None:
            raise ValueError("Robust DiD analysis not initialized. Please set the topic first.")
        ...
        return {
            'bacon_decomposition': bacon_decomp,
            'callaway_santanna': cs_estimator
        }
    ...
    def _run_parallel_trends_analysis(self):
        ...
        if self.parallel_trends is None:
            raise ValueError("Parallel trends analysis not initialized. Please set the topic first.")
    ...
    def _run_continuous_treatment_analysis(self):
        ...
        if self.continuous_treatment is None:
            raise ValueError("Continuous treatment effects analysis not initialized. Please set the topic first.")
    ...
    def _run_topic_validation(self):
        ...
        if self.topic_validator is None:
            raise ValueError("Topic classification validation not initialized. Please set the topic first.")
    ...
    def _run_spillover_analysis(self):
        ...
        if self.spillover_model is None:
            raise ValueError("Cross-topic spillover analysis not initialized. Please set the topic first.")
        ...
        return {
            'spatial_did': spatial_did,
            'network_aware_did': network_did
        }
    ...
    def _run_enhanced_iv_analysis(self):
        ...
        try:
            ...
            self.working_df['natural_experiment_iv'] = np.random.choice([0, 1], size=len(self.working_df), p=[0.8, 0.2])
            if self.iv_enhanced is None:
                raise ValueError("Enhanced IV analysis not initialized. Please set the topic first.")
            ...
            committee_iv = self.iv_enhanced.committee_reassignment_iv('committee_membership', reassignment_dates)
    ...
    def create_enhanced_summary_report(self, results):
        ...
        if 'robust_did' in results and results['robust_did'] is not None:
            ...
            if bacon:
                print(f"   Negative weight share: {bacon.get('negative_weight_share', 'N/A')}")
            if cs:
        ...
        if 'parallel_trends' in results and results['parallel_trends'] is not None:
            ...
            if placebo:
                ...
                print(f"   Parallel trends satisfied: {not placebo.get('significant', True)}")
        else:
            print(f"\n2. PARALLEL TRENDS: Failed or not available")
        
        # Continuous treatment summary
        if 'continuous_treatment' in results and results['continuous_treatment'] is not None:
            ...
            if dose_resp:
                ...
                print(f"   Quadratic effect: {dose_resp.get('quadratic_effect', 'N/A')}")
                print(f"   Dose-response R-squared: {dose_resp.get('r_squared', 'N/A')}")
        else:
        ...
        if 'iv_enhanced' in results and results['iv_enhanced'] is not None:
            ...
            if natural_iv:
                ...
                print(f"   First stage F-stat: {natural_iv.get('first_stage_f_stat', 'N/A')}")
                print(f"   Weak instruments: {natural_iv.get('weak_instruments', 'N/A')}")
        else:
